...
class Sequential:
    ...
    def compute_loss(self, A, y):
        if self.type == 'relu':
            return (1 / len(A)) * (np.square(np.subtract(y, A)))
        if self.type == 'sigmoid': # TODO Double check this calculation, I just copy and pasted from the internet
            y_pred = np.clip(A, 1e-7, 1 - 1e-7)
            term_0 = (1-y) * np.log(1-y_pred + 1e-7)
            term_1 = y * np.log(y_pred + 1e-7)
            return -np.mean(term_0+term_1, axis=0)
        if self.type == 'softmax':
            pass # gotta finish this
